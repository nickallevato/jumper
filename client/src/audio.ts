...
export type SfxKey = "jump" | "land" | "join" | "chain";
...
const SYNTH_SFX_SPECS: Record<Exclude<SfxKey, "chain">, ToneSpec> = {
  // Soft upward chirp
...
export class AudioBus {
  ...
  preload(): void {
    for (const key of Object.keys(SYNTH_SFX_SPECS) as Exclude<SfxKey, "chain">[]) {
      const url = URL.createObjectURL(synthesizeWavBlob(SYNTH_SFX_SPECS[key]));
      this.scene.load.audio(key, url);
    }
    this.scene.load.audio("chain", ["/audio/chain.ogg", "/audio/chain.m4a"]);
  }
  ...
  play(key: SfxKey, opts: { volume?: number; detune?: number } = {}): void {
    if (!this.ready || this.muted) return;
    const sound = this.scene.sound;
    if (!sound.get(key) && !this.scene.cache.audio.exists(key)) return;
    sound.play(key, { volume: opts.volume ?? 1, detune: opts.detune ?? 0 });
  }
